...
`DispatchRouteMiddleware` resolves route middleware on every request through the configured factory. The container and middleware resolver determine object lifetimes; compiled route collections preserve the same resolution and execution behavior.
...
`CompiledRoutes` loads an explicitly compiled route collection from a generated cache file.
...
- route handler definitions and middleware groups are initialized lazily.

The cache file is generated by `RouteCacheGenerator`, normally through `componenta/router-app` and `app:build`. Production service factories load the map when optimization is enabled. Rebuild it whenever route definitions, configuration or external inputs used by the route source change. Combined expressions are checked against PCRE limits and oversized groups are split further. If a combined expression exhausts a matching limit, matching continues through the original individual route patterns. `CachedRouteLocator` falls back to its supplied source when generated data is invalid, including when the application converts warnings into exceptions.

Replacing a compiled collection's public `compiler` before its first match uses the source matching path. URL generation uses the current compiler while retaining any explicitly configured generation syntax. Cache format 8 records whether that syntax was explicit; `tryFromCache()` rejects older formats so its caller can use the source.
...
| `ConfigKey::COMPILED_PIPELINE` | Enables the compiled route map in production; defaults to `true`. |
