...
beforeEach(function () {
    $this->syntax = new CurlySyntax();
});

// --- canParse ---

it('detects curly brace parameters', function (string $pattern, bool $expected) {
...
it('parses constraint with default', function () {
    $result = $this->syntax->parse('/posts/{page:\d+=1}');

    expect($result['optionalParameters'])->toBe(['page' => '\d+'])
        ->and($result['defaults'])->toBe(['page' => '1'])
        ->and($result['tokens'])->toBe(['page' => '\d+']);
});

it('parses multiple parameters', function () {
    $result = $this->syntax->parse('/users/{id}/posts/{slug}');

    expect($result['parameters'])->toBe(['id' => null, 'slug' => null]);
});
...
it('compiles required parameter with token override', function () {
...
it('matches url without optional parameter', function () {
    ...
    expect(preg_match($regex, '/users', $m))->toBe(1);
});
...
it('compiles optional parameter in middle of path', function () {
    $regex = '#^' . $this->syntax->toRegex('/users/{id?}/posts', []) . '$#';

    expect(preg_match($regex, '/users/posts'))->toBe(1)
        ->and(preg_match($regex, '/users/5/posts'))->toBe(1);
});

it('compiles multiple optional parameters', function () {
    $regex = '#^' . $this->syntax->toRegex('/posts/{category?}/{page?}', []) . '$#';

    expect(preg_match($regex, '/posts'))->toBe(1)
        ->and(preg_match($regex, '/posts/php'))->toBe(1)
        ->and(preg_match($regex, '/posts/php/2'))->toBe(1);
});
...
it('throws on parameter value not matching token', function () {
    expect(fn () => $this->syntax->buildPath('/users/{id}', ['id' => 'abc'], ['id' => '\d+'], [], 'test'))
        ->toThrow(InvalidArgumentException::class);
});
